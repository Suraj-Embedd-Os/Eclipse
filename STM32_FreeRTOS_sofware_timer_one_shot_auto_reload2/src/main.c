...
static void prvTimerCallBackFun(TimerHandle_t xtimer);
...
int main(void)
{
	...
	xOneShotTimer=xTimerCreate("oneShot",mainONE_SHOT_TIMER_PERIOD,pdFALSE,0,prvTimerCallBackFun);

	xAutoReloadTimer=xTimerCreate("periodic",mainAUTO_RELOAD_TIMER_PERIOD,pdPASS,0,prvTimerCallBackFun);
	...
	if(xOneShotTimer!=NULL && xAutoReloadTimer!=NULL )
	{
		...
		if(xTimer1Started!=0 && xTimer2Started!=0)
		{
...
static void prvTimerCallBackFun(TimerHandle_t xtimer)
{
	...
	{
    sprintf(usr_msg,"One-shot timer callback executing: %d\r\n",xTimeNow);
  	printmsg(usr_msg);
	}

	else
	{
		 sprintf(usr_msg,"periodic timer callback executing: %d\r\n",xTimeNow);
		  printmsg(usr_msg);
